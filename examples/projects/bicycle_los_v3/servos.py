"""Fused velocity / heading / yaw-rate servos for MPC-guided cascade."""
...
class Servos(DynamicSystem):
    """Velocity, heading, and yaw-rate cascade ending at force and steer.
      ...
      Steer feedforward ``delta_ff`` comes from MPC ``x_nom``; no kinematic FF
    inside this block.

      State ``x = [int_v, filt_v, int_h, filt_h, int_r, filt_r]``.

      Ports ``heading_ref``, ``vx_ref``, ``delta_ff``, ``y`` → ``F_rear``, ``delta``.
    """
    ...
    def __init__(
        ...
        self.add_input_port("delta_ff", nominal_value=np.array([0.0]))
        self.add_input_port("y", nominal_value=np.zeros(10))
        self.add_output_port(
            "F_rear",
            dim=1,
            function=self.h_F_rear,
            dependencies=("heading_ref", "vx_ref", "delta_ff", "y"),
        )
        self.add_output_port(
            "delta",
            dim=1,
    ...
    def h_F_rear(self, x, u, t=0.0, params=None):
        F_rear, _ = self._cascade(x, u, params=params)
        return np.array([F_rear], dtype=float)

    def h_delta(self, x, u, t=0.0, params=None):
        _, delta = self._cascade(x, u, params=params)
