"""High-speed (≈20 m/s) MPC obstacle avoidance on the rate-input bicycle.

Uses :class:`~minilink.dynamics.catalog.vehicles.dynamic_bicycle.JaxDynamicBicycleRateInputs`
so wheel speed and steer angle are states and MPC commands their rates — smoother
than direct ``w_rear`` / ``delta`` inputs at high speed. A circular keep-out zone
on the centerline is enforced via ``CallableSet`` on ``PlanningProblem.X``.

Back-of-envelope sizing (``r_r = 0.3`` m, wheelbase ``a + b = 2`` m)::

    w_rear_ref = U_TARGET / r_r ≈ 67 rad/s   (< W_REAR_MAX = 120)
    lookahead  = U_TARGET * MPC_HORIZON ≈ 70 m  (horizon 3.5 s @ 20 m/s)
    obstacle   @ x = 110 m  → enters MPC view at x ≳ 40 m (t ≳ 2 s)
    start      @ y = 4.5 m → 1.8 m above keep-out disk (R = 2.7 m) on centerline
    soft Q_y   → delay merge to y = 0 until past the obstacle
...
# --- Cost ---
# Soft lateral weight (Q_y = 6) avoids premature merge into the keep-out zone.
...
cost = QuadraticCost.from_system(
    ...
    R=np.diag([0.5, 20.0]),
    S=np.diag([0.0, 35.0, 45.0, 3.0, 12.0, 16.0, 0.05, 80.0]),
    xbar=x_ref,
    ubar=ubar,
)
...
print("MPC high-speed obstacle avoidance (rate inputs)")
print(
    f"  u_target={U_TARGET} m/s, mpc_hz={MPC_HZ}, sim_hz={SIM_HZ}, "
    f"horizon={MPC_HORIZON}s, backend={MPC_BACKEND}"
)
print(
    f"  obstacle center={OBSTACLE_CENTER}, radius={OBSTACLE_RADIUS} "
    f"(+ margin {OBSTACLE_MARGIN}), start y={Y0}"
)
print(
    f"  lookahead ≈ {U_TARGET * MPC_HORIZON:.0f} m, "
    f"w_rear_ref ≈ {w_rear_ref:.1f} rad/s"
)
