...
def main():
    ...
    vehicle = create_vehicle(Y=10.0, vx=0.0, theta=np.pi, tire_slip_mode=None)

    diagram = create_diagram(vehicle, vx_ref=vx)

    diagram.plot_diagram()

    diagram.compute_trajectory(tf=10, dt=0.01)
    ...
    plt.xlabel("X [m]")
    plt.ylabel("Y [m]")
    plt.title("LOS control point trajectory over time")
    plt.legend()
    plt.grid(True)
    plt.axis("equal")
    plt.show()
