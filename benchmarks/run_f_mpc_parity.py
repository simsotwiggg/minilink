...
Usage (from repo root)::
    ...
    python benchmarks/run_f_mpc_parity.py --capture   # write baselines/f_mpc_parity.json
    python benchmarks/run_f_mpc_parity.py             # compare vs baseline, exit 1 on fail
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--capture",
        action="store_true",
        help="Write baselines/f_mpc_parity.json from the current run",
    )
    parser.add_argument(
        "--baseline",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Speed regression factor override (default 2.0)",
    )
    parser.add_argument(
        "--host-hint",
        default=None,
    ...
    recorded = run_f_mpc_parity_suite()
